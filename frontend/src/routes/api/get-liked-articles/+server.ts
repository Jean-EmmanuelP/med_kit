...
export const GET = async ({ url, locals: { supabase, safeGetSession } }) => {
    ...
    console.log(`Calling RPC get_liked_articles with: user=${userId}, discipline=${specialty || null}, subDiscipline=${subDiscipline || null}, offset=${offset}, search=${search || null}`);
    ...
    const { data: articlesData, error: rpcError } = await supabase.rpc(
        ...
        {
            p_user_id: userId,
            p_discipline_name: specialty || null,         // <<< Pass discipline name
            p_sub_discipline_name: subDiscipline || null, // <<< Pass sub-discipline name
            p_offset: offset,
