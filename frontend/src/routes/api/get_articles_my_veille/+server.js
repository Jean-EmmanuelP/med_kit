...
export const GET = async ({ url, locals: { supabase, safeGetSession } }) => { // Add safeGetSession
    // --- Read parameters ---
    ...
    console.log(`Calling RPC get_all_articles_sub_disciplines with: discipline=${specialty}, sub_discipline=${subDiscipline || null}, offset=${offset}, search=${search || null}, user=${userId}`);
    ...
    const { data: articlesData, error: rpcError } = await supabase.rpc(
        'get_all_articles_sub_disciplines', // RPC name
        {
            ...
            p_user_id: userId // <<< Pass user ID (or null)
        }
    );
