...
async function handleResetPassword(e) {
  ...
  const { error } = await supabase.auth.resetPasswordForEmail(email, {
    redirectTo: 'https://veillemedicale.fr/update-password'
  });
  if (error) {
    errorMessage = $i18n.login.resetPasswordError;
  } else {
    successMessage = $i18n.login.resetPasswordSuccess;
  }
  isLoading = false;
}
</script>
...
<main class="flex min-h-screen items-center justify-center bg-black px-4 py-12">
  <div class="w-full max-w-md rounded-2xl bg-white p-8 shadow-2xl">
    <h1 class="mb-6 text-center text-3xl font-bold tracking-tight capitalize">{$i18n.login.resetPasswordTitle}</h1>
    <p class="mb-4 text-center text-gray-600">{$i18n.login.resetPasswordInstruction}</p>
    ...
    <form class="space-y-6" on:submit={handleResetPassword}>
      ...
      >
        ...
        {:else}
          {$i18n.login.resetPasswordButton}
        {/if}
      </button>
    </form>
  </div>
